...
func (*server) UpdateBlog(ctx context.Context, req *blogpb.UpdateBlogRequest) (*blogpb.UpdateBlogResponse, error) {
	...
	if err := res.Decode(data); err != nil {
		return nil, status.Errorf(codes.NotFound,
			fmt.Sprintf("Cannot find blog with specific ID: %v\n", err),
		)
	}
...
func (*server) DeleteBlog(ctx context.Context, req *blogpb.DeleteBlogRequest) (*blogpb.DeleteBlogResponse, error) {
	...
	if err != nil {
		return nil, status.Errorf(
			codes.Internal,
			fmt.Sprint("Cannot delete object in MongoDB: %v", err),
		)
	}
...
func main() {
	// If our system crash, we get the file name and line number
	log.SetFlags(log.LstdFlags | log.Lshortfile)

	fmt.Println("Welcome to blog server!")
	...
	if tls {
		...
		if sslErr != nil {
			log.Fatalf("Failed loading certifications: %v", sslErr)
			return
		}
